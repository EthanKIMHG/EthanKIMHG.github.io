...
**Overview**  
중소벤처기업부 산학연 컨소시엄 R&D 과제(블록체인 기반 대·중·소기업 공유 데이터 보안관리 기술)의 일환으로, 국내 대기업 파트너사와 협업해 스마트팩토리 환경에서 기업 간 공유되는 데이터의 접근 권한을 통제하는 DID 신원 인증 플랫폼을 개발했습니다. Holder, Issuer, Admin, Audit Admin으로 구성된 프론트엔드 구조를 설계하고 구현했으며, 단순 화면 개발을 넘어 인증/인가 흐름, 감사 로그, 데이터 시각화, 폐쇄망 배포, GS 인증 대응까지 포함해 실제 운영 가능한 구조를 만드는 데 집중했습니다.
...
### 4. 확인된 사실만 기록하고, 변경은 추적 가능한 단위로 남깁니다

밸리데이터 활성화나 자금 이동처럼 사후 감사를 받는 영역에서는, 확인되지 않은 값을 그럴듯하게 채우기보다 TBD로 남겨두는 편이 안전하다고 생각합니다. 형상관리대장에 모든 변경을 번호를 매겨 append-only로 기록하고 Git 커밋과 1:1로 연결해, 언제 무엇을 왜 바꿨는지 사후에도 재구성할 수 있게 관리합니다.

### 5. 프론트엔드를 제품의 가장자리까지 확장합니다
